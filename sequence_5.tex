...
\subsection{De la série de Fourier à la Transformée de Fourier Discrète}
\begin{frame}
...
\subsection{Transformées de Fourier Discrète et Rapide : TFD et TFR (ou FFT)}
\begin{frame}
\begin{exampleblock}{Exercice \Roman{exampleBlockCounter} - Spectres monolatéraux avec Python}
Représenter les spectres monolatéraux des signaux suivants avec Python en utilisant la fonction fft du package numpy.fft :
\begin{itemize}
  \item $s_1(t) = 3$
...
\end{itemize}
Le but est bien sûr d'obtenir le même résultat que la séquence précédent. Il va falloir manipuler le résultat de la fonction fft sous python pour y parvenir ! 
\end{exampleblock}
\end{frame}

\begin{frame}
\begin{alertblock}{Remarques importantes}
\begin{itemize}
\item La FFT est un algorithme optimisé de la TFD
\item Cet algorithme ne donne pas un résultat directement exploitable
\item Il faut manipuler ce résultat pour retrouver les spectres monolatéraux que l'on avait obtenu
\item Pour obtenir une compréhension globale, il faut aussi se pencher sur les parités des spectres d'un signal réel et introduire la notion de spectre bilatéral
\end{itemize}
\justifying
...
\subsection{Représentation bilatérale}
\begin{frame}
\begin{exampleblock}{Exercice \Roman{exampleBlockCounter} - Signal réel et parité des spectres}
...
\begin{frame}
\stepcounter{exampleBlockCounter}
\begin{exampleblock}{Exercices \Roman{exampleBlockCounter} - Spectres bilatéraux avec Python}
Représenter les spectres bilatéraux des signaux suivants avec Python:
\begin{itemize}
  \item $s_1(t) = 3$
  \item $s_2(t) = 6sin(2\pi t)$
...
\stepcounter{exampleBlockCounter}
\begin{frame}
\begin{exampleblock}{Exercice \Roman{exampleBlockCounter} - Des spectres bilatéraux à l'expression temprorelle}
...
\subsection{Représentation monolatérale et bilatérale de la note}
\stepcounter{exampleBlockCounter}
\begin{frame}
\begin{exampleblock}{Exercice \Roman{exampleBlockCounter} - Retour sur la note}
\itemize\justifying
...
\item Tracer les spectres monolatéraux et bilatéraux de ce signal. Quelles informations en tirez-vous ?
\end{exampleblock}
\end{frame}
...
\section{Transition vers le filtrage fréquentiel}
\subsection{Transformée en Z}
\stepcounter{exampleBlockCounter}
\begin{frame}
\begin{block}{Transformée en Z}
En posant $z=e^{2i\pi \frac{k}{N}}$ dans l'expression de la TFD et en ne tenant pas compte du facteur $\frac{1}{N}$, on obtient la transformée en $Z$ :
\begin{align*}
S(z) = \sum_{n=0}^{N-1}s(n)z^{-n}
\end{align*}
Cette transformée sera utilisée dans la prochaine séquence pour exprimer la fonction de transfert d'un système numérique.
\end{block}
\begin{exampleblock}{Exercice \Roman{exampleBlockCounter} - Théorème du retard}
	\justifying
